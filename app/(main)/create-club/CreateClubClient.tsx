...
interface CreateClubClientProps {
  userId: string;
  initialCourts: Court[];
}
...
export default function CreateClubClient({
  ...
  const handleCreateLocationSubmit = async () => {
    try {
      ...
      // Use the new API route instead of Server Action to ensure the request URL is /api/courts
      ...
      if (!response.ok || result.error) {
        ...
        // Add new court to the list and select it in the form
        ...
        if (newCourtId && result.name) {
          ...
          // Automatically select the new location in the active schedule row
          ...
          // If API doesn't return the object, we might need to refetch
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!response.ok || result.error) {
        message.error(result.error || 'Failed to create club');
      } else {
